...
class GamificationService:
    ...
    def handle_tool_usage(self, tool_name: str, action: str) -> GamificationResult:
        ...
        if tool_name.lower() == "tests" and action.lower() == "run":
            if not self.progress.has_completed_event("run-tests"):
                run_event = XPEvent(
                    id="run-tests",
                ...
                if not self.progress.has_completed_quest("run-tests"):
                    quest_result = self.complete_quest("run-tests")
                    result.completed_quests.extend(quest_result.completed_quests)
                    result.earned_badges.extend(quest_result.earned_badges)
