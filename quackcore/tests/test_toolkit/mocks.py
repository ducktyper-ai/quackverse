...
class BaseMockTool(BaseQuackToolPlugin):
    ...
    def __init__(self, name: str, version: str) -> None:
        ...
        patch_targets = [
            # Filesystem
            ("quackcore.fs.service.get_service", create_mock_fs()),
            # Logging
            ("quackcore.config.tooling.logger.setup_tool_logging", MagicMock()),
            ("quackcore.config.tooling.logger.get_logger",
             MagicMock(return_value=MockLogger())),
            # OS
            ("os.getcwd", MagicMock(return_value=tempfile.gettempdir())),
            # FileWorkflowRunner
            ("quackcore.workflow.runners.file_runner.FileWorkflowRunner",
             MockWorkflowRunner),
        ]
