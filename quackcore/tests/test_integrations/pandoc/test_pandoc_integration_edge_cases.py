...
def test_integration_with_custom_config_path():
    ...
    with patch('quackcore.paths.service', MagicMock()), \
            patch('quackcore.integrations.pandoc.config.PandocConfigProvider',
                  return_value=mock_config_provider), \
            patch('quackcore.fs.service.standalone.resolve_path',
...
def test_integration_initialize_with_invalid_config():
    ...
    with patch(
            'quackcore.integrations.pandoc.config.PandocConfigProvider.validate_config',
            return_value=False):
...
@patch('quackcore.integrations.pandoc.operations.get_file_info')
@patch('quackcore.fs.service.standalone')
