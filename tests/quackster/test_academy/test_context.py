# tests/quackster/test_academy/test_context.py
"""
...
class TestTeachingConfig:
    ...
    def test_ensure_course_id(self):
        ...
        config = TeachingConfig(
            course_name="Test Course", github=GitHubConfig(organization="test-org")
        )
...
class TestTeachingContext:
    ...
    def test_github_property_not_available(
        self, teaching_context, mock_integration_registry
    ):
    ...
    def test_github_property_initialization_error(
        self, teaching_context, mock_integration_registry
    ):
        ...
        mock_github.initialize.return_value = MagicMock(
            success=False, error="Initialization error"
        )
    ...
    def test_from_config_success(self, mock_fs, mock_resolver, teaching_config):
        ...
        with patch("quackcore.config.loader.load_yaml_config") as mock_load_config:
            ...
            assert (
                context.config.github.organization
                == teaching_config.github.organization
            )
    ...
    def test_from_config_env_var(
        self, monkeypatch, mock_fs, mock_resolver, teaching_config
    ):
    ...
    def test_from_config_search_paths(
        self, monkeypatch, mock_fs, mock_resolver, teaching_config
    ):
                ...
                with patch("pathlib.Path.cwd") as mock_cwd:
                    ...
                    with patch("pathlib.Path.home") as mock_home:
                        ...
                        assert mock_load_config.call_args[0][0] == Path(
                            "/current/dir/config/quackster.yaml"
                        )
    ...
    def test_from_config_no_config_found(self, monkeypatch, mock_fs, mock_resolver):
        ...
        with patch("pathlib.Path.exists") as mock_exists:
            ...
            assert "No quackster configuration file found" in str(exc_info.value)
    ...
    def test_from_config_load_error(self, mock_fs, mock_resolver):
        ...
        with patch("quackcore.config.loader.load_yaml_config") as mock_load_config:
            ...
            assert "Failed to load quackster configuration" in str(exc_info.value)
