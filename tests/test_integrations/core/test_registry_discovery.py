...
class TestIntegrationRegistryDiscovery:
    ...
    def test_discover_integrations(self, registry):
        ...
        mock_entry_points = [
            MockEntryPoint(
                "integration1",
                "quackcore.integrations.integration1",
                lambda: MockIntegration("Integration1"),
            ),
            MockEntryPoint(
                "integration2",
                "quackcore.integrations.integration2",
        ...
        with patch(
            "quackcore.integrations.core.registry.IntegrationRegistry._get_entry_points"
        ) as mock_get_eps:
            mock_get_eps.return_value = mock_entry_points

            # Test discovery with no plugin loader
            with patch(
                "quackcore.integrations.core.registry.IntegrationRegistry._get_plugin_loader"
            ) as mock_get_loader:
    ...
    def test_get_entry_points(self, registry):
        ...
        mock_eps = [
            EntryPoint(
                name="integration1",
                value="quackcore.integrations.integration1",
                group="quackcore.integrations",
            ),
            EntryPoint(
                name="integration2",
                value="quackcore.integrations.integration2",
                group="quackcore.integrations",
            ),
        ]
