...
class TestMarkdownToDocxOperations:
    ...
    def test_convert_markdown_to_docx_once(self, config, mock_fs):
        ...
        with patch(
                "quackcore.integrations.pandoc.operations.md_to_docx.prepare_pandoc_args") as mock_args:
            ...
            with patch("pypandoc.convert_file") as mock_convert:
                ...
                _convert_markdown_to_docx_once(markdown_path, output_path,
                                                          config)

                mock_fs.create_directory.assert_called_with(output_path.parent,
                                                            exist_ok=True)
                mock_args.assert_called_with(config, "markdown", "docx",
                                             config.md_to_docx_extra_args)
        ...
        with patch(
            ...
            with patch("pypandoc.convert_file") as mock_convert:
                ...
                with pytest.raises(QuackIntegrationError) as excinfo:
                    _convert_markdown_to_docx_once(markdown_path,
                                                              output_path, config)
    ...
    def test_get_conversion_output(self, mock_fs):
        ...
        with patch(
                "quackcore.integrations.pandoc.operations.md_to_docx.time.time") as mock_time:
    ...
    def test_convert_markdown_to_docx(self, config, metrics, mock_fs):
        ...
        # Test successful conversion
        with patch(
                "quackcore.integrations.pandoc.operations.md_to_docx._validate_markdown_input") as mock_validate:
            mock_validate.return_value = 512  # Original size

            with patch(
                    "quackcore.integrations.pandoc.operations.md_to_docx._convert_markdown_to_docx_once") as mock_convert:
                # mock_convert doesn't return anything

                with patch(
                        "quackcore.integrations.pandoc.operations.md_to_docx._get_conversion_output") as mock_output:
                    mock_output.return_value = (
                    1.0, 10240)  # (conversion_time, output_size)

                    with patch(
                            "quackcore.integrations.pandoc.operations.md_to_docx.validate_conversion") as mock_validate_out:
                        mock_validate_out.return_value = []  # No validation errors

                        with patch(
                                "quackcore.integrations.pandoc.operations.md_to_docx.track_metrics") as mock_track:
                            # Test successful conversion
                            result = convert_markdown_to_docx(markdown_path,
                                                              output_path, config,
                                                              metrics)
                            ...
                            mock_convert.assert_called_once_with(markdown_path,
                                                                 output_path, config)
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        result = convert_markdown_to_docx(markdown_path, output_path,
                                                          config, metrics)
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                                "quackcore.integrations.pandoc.operations.md_to_docx.time.sleep") as mock_sleep:
                            # Test retry logic
                            result = convert_markdown_to_docx(markdown_path,
                                                              output_path, config,
                                                              metrics)
        ...
        with patch(
            ...
            result = convert_markdown_to_docx(markdown_path, output_path, config,
                                              metrics)
    ...
    def test_validate_conversion(self, mock_fs, config):
        ...
        with patch(
                "quackcore.integrations.pandoc.operations.md_to_docx.check_file_size") as mock_size:
            mock_size.return_value = (True, [])

            with patch(
                    "quackcore.integrations.pandoc.operations.md_to_docx.check_conversion_ratio") as mock_ratio:
                mock_ratio.return_value = (True, [])

                with patch(
                        "quackcore.integrations.pandoc.operations.md_to_docx.validate_docx_structure") as mock_validate:
                    mock_validate.return_value = (True, [])

                    with patch(
                            "quackcore.integrations.pandoc.operations.md_to_docx._check_docx_metadata") as mock_metadata:
                        validation_errors = validate_conversion(
                            output_path, input_path, original_size, config
                        )

                        assert len(validation_errors) == 0
                        mock_size.assert_called_with(output_info.size,
                                                     config.validation.min_file_size)
                        mock_ratio.assert_called_with(output_info.size, original_size,
                                                      config.validation.conversion_ratio_threshold)
                        mock_validate.assert_called_with(output_path,
                                                         config.validation.check_links)
                        mock_metadata.assert_called_with(output_path, input_path,
                                                         config.validation.check_links)
        ...
        with patch(
            ...
            with patch(
                ...
                False, ["Conversion ratio is below threshold"])
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    False, ["DOCX document has no paragraphs"])
    ...
    def test_check_docx_metadata(self, mock_fs):
        ...
        with patch("docx.Document", return_value=mock_doc):
            ...
            with patch.object(md_to_docx, "logger", create=True) as mock_logger:
                ...
                assert "Source file reference missing" in mock_logger.debug.call_args[0][0]
        ...
        with patch("docx.Document", side_effect=Exception("DOCX error")):
            ...
            with patch.object(md_to_docx, "logger", create=True) as mock_logger:
                _check_docx_metadata(output_path, source_path, check_links)
                mock_logger.debug.assert_called_once()
                assert "Could not check document metadata" in mock_logger.debug.call_args[0][0]