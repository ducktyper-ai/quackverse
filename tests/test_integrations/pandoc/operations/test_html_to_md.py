...
from quackcore.fs.results import FileInfoResult, OperationResult, ReadResult, WriteResult
...
class TestHtmlToMarkdownOperations:
    ...
    def test_attempt_conversion(self, config):
        ...
        with patch("pypandoc.convert_file") as mock_convert:
            ...
            # Test conversion
            with patch(
                    "quackcore.integrations.pandoc.operations.html_to_md.prepare_pandoc_args") as mock_args:
                mock_args.return_value = ["--strip-comments", "--no-highlight"]

                with patch(
                        "quackcore.integrations.pandoc.operations.html_to_md.post_process_markdown") as mock_post:
    ...
    def test_write_and_validate_output(self, mock_fs, config):
        ...
        with patch(
                "quackcore.integrations.pandoc.operations.html_to_md.validate_conversion") as mock_validate:
            ...
            mock_fs.create_directory.assert_called_with(output_path.parent,
                                                        exist_ok=True)
            mock_fs.write_text.assert_called_with(output_path, cleaned_markdown,
                                                  encoding="utf-8")
            mock_validate.assert_called_with(output_path, input_path, original_size,
                                             config)
    ...
    def test_convert_html_to_markdown(self, config, metrics, mock_fs):
        ...
        with patch(
                "quackcore.integrations.pandoc.operations.html_to_md._validate_input") as mock_validate:
            mock_validate.return_value = 1024  # Original size

            with patch(
                    "quackcore.integrations.pandoc.operations.html_to_md._attempt_conversion") as mock_attempt:
                mock_attempt.return_value = "# Test\n\nContent"

                with patch(
                        "quackcore.integrations.pandoc.operations.html_to_md._write_and_validate_output") as mock_write:
                    mock_write.return_value = (
                    1.0, 100, [])  # (conversion_time, output_size, validation_errors)

                    with patch(
                            "quackcore.integrations.pandoc.operations.html_to_md.track_metrics") as mock_track:
                        # Test successful conversion
                        result = convert_html_to_markdown(html_path, output_path,
                                                          config, metrics)
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                            "quackcore.integrations.pandoc.operations.html_to_md.track_metrics") as mock_track:
                        with patch(
                                "quackcore.integrations.pandoc.operations.html_to_md.time.sleep") as mock_sleep:
                            # Test retry logic
                            result = convert_html_to_markdown(html_path, output_path,
                                                              config, metrics)
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        assert "Conversion validation failed after maximum retries" in result.error
