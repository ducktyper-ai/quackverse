...
from . import badges, github_api, xp
from .models import Quest, UserProgress
...
def _verify_run_ducktyper(user: UserProgress) -> bool:
    ...
    return user.has_completed_event("run-ducktyper")
...
def _verify_daily_streak(user: UserProgress) -> bool:
    ...
    return (
        len([e for e in user.completed_event_ids if e.startswith("run-ducktyper-day-")])
        >= 3
    )
...
_QUESTS["run-ducktyper"].verify_func = _verify_run_ducktyper
